...
<x-app-layout>
                        ...
                        <div class="space-y-6">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 pb-6">
                                ...
                                <div>
                                    ...
                                    <p class="text-xs text-gray-500 mt-1">Admin memiliki akses ke semua cabang</p>
                                    @error('role')
                                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                                    @enderror
                                </div>
                                ...
                                <div id="cabang-field">
                                    ...
                                    <select name="id_cabang" id="id_cabang"
                                        ...
                                        <option value="{{ $item->id }}" {{ old('id_cabang', $user->id_cabang) == $item->id ? 'selected' : '' }}>
                                            {{ $item->nama_cabang }}
                                        </option>
                                        @endforeach
                                    </select>
                                    <p class="text-xs text-gray-500 mt-1" id="cabang-info">Wajib dipilih untuk role Staff</p>
                                    @error('id_cabang')
                                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                                    @enderror
                                </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // Ambil Value Enum dari PHP agar aman
            const ROLE_ADMIN = "{{ \App\Enums\Role::ADMIN->value }}";
            const ROLE_STAFF = "{{ \App\Enums\Role::STAFF->value }}";
            ...
            function toggleCabangField() {
                ...
                if (selectedRole === ROLE_ADMIN) {
                    ...
                    cabangInfo.classList.remove('text-gray-500', 'text-blue-600');
            ...
            // Jalankan saat page load untuk mengatur state awal berdasarkan data user yang sedang diedit
            if (roleSelect.value) {
                toggleCabangField();
            }
        });
