    ...
    <div class="py-6">
        <div class="max-w-8xl mx-auto px-4 sm:px-6 lg:px-8 mb-6">
            <h2 class="text-2xl font-bold leading-7 text-gray-900 sm:text-3xl">
                {{ __('Data Transaksi Pelanggan') }} - Administrator
            </h2>
        </div>
                    ...
                    <form method="GET" action="{{ route('admin.transaksi.index') }}">
                        <div class="flex flex-col lg:flex-row lg:items-end gap-3 flex-wrap">
                            ...
                            <div class="w-full lg:w-48">
                                ...
                                <select name="id_cabang" class="w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">
                                    <option value="">Semua Cabang</option>
                                    @foreach($cabangs as $cabang)
                                    <option value="{{ $cabang->id }}" {{ request('id_cabang') == $cabang->id ? 'selected' : '' }}>
                                        {{ $cabang->nama_cabang }}
                                    </option>
                                    @endforeach
                                </select>
                    ...
                    <table class="min-w-full divide-y divide-gray-200">
                        <thead class="bg-gray-100">
                            <tr>
                                ...
                                <th class="px-4 py-3 text-center text-xs font-bold text-gray-600 uppercase">Cabang</th>
                                ...
                                <th class="px-6 py-3 text-center text-xs font-bold text-gray-600 uppercase">Tipe</th>
                                <th class="px-6 py-3 text-center text-xs font-bold text-gray-600 uppercase">Alamat</th>
                                <th class="px-6 py-3 text-center text-xs font-bold text-gray-600 uppercase">Info</th>
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            ...
                            <tr class="hover:bg-gray-50">
                                ...
                                {{-- Cabang (KHUSUS ADMIN) --}}
                                <td class="px-4 py-3 text-left text-sm font-semibold text-gray-700">
                                    {{ $item->cabang->nama_cabang ?? '-' }}
                                </td>

                                {{-- Waktu (Gabungan Hari & Tanggal untuk hemat tempat) --}}
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-center text-sm font-medium">
                                    <div class="flex justify-center items-center space-x-3">
                                        ...
                                        {{-- Edit (Admin biasanya punya akses edit) --}}
                                        ...
                                        {{-- Hapus --}}
                                        <form action="{{ route('admin.transaksi.destroy', $item->id) }}" method="POST" onsubmit="return confirm('Yakin ingin menghapus transaksi ini dari Database?')" class="inline-block">
                                            @csrf
                                            @method('DELETE')
